...
public class OssUploadService implements UploadService {
    ...
    public ResponseResult uploadImg(MultipartFile img) throws IOException {
       ...
       String url = uploadOss(img,filePath);//使用七牛云进行头像上传  2099/2/3/wqeqeqe.png
       // String url = uploadFile(filePath,img.getInputStream());//使用阿里云进行头像上传  2099/2/3/wqeqeqe.png

        //返回上传到服务器上的的图片的外链(即访问地址)给前端。这样前端可以去访问并显示已上传的头像
        ...
        try {
            ...
            try {
                ...
                System.out.println(putRet.hash);
                  //返回上传到服务器上的的图片的外链(即访问地址)
                return urlLcation+key;
                //return "http://rmktq8jpn.bkt.clouddn.com/"+key;
